...
print(f"Unique epochs: {df_epochs['epoch_idx'].n_unique()}")
...
- **epoch_idx**: Index identifying each epoch
...
metadata_cols = [col for col in df_epochs.columns 
                 if col not in ['time', 'epoch_idx', 'sample_idx'] 
...
print(df_with_meta.select(['epoch_idx', 'condition', 'trial_num', 'rt', 'correct']).unique())
```
...
**Returns**: Polars DataFrame with columns for each channel (in µV), time, epoch_idx, sample_idx, and any metadata columns
