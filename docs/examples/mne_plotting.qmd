...
Compare ERPs across different conditions:

```{python}
# Create epochs for multiple conditions
event_id = {
    'auditory/left': 1,
    'auditory/right': 2,
    'visual/left': 3,
...
# Plot auditory condition
epochs_aud = epochs_all['auditory/left']
fig_aud = plot_evoked(epochs_aud, time_topo=[0.1, 0.2])
fig_aud.update_layout(title="Auditory Left Condition", height=650, width=1000)
fig_aud.show()
```
...
for ch in epochs.ch_names:
    if any(prefix in ch for prefix in ['Fp', 'AF', 'F']):
        custom_colors[ch] = '#1f77b4'  # Blue for frontal
    elif any(prefix in ch for prefix in ['P', 'O', 'PO']):
        custom_colors[ch] = '#d62728'  # Red for posterior
    else:
        custom_colors[ch] = '#7f7f7f'  # Gray for others

# Plot with custom colors
fig = plot_evoked(epochs, cmap=custom_colors)
fig.update_layout(
    title="ERPs - Custom Channel Coloring",
    height=600,
    width=900
)
fig.show()
